...
gulp.task("copy-libs", async function () {
  gulp
    ...
    .pipe(gulp.dest("mysite/static/vendors"));
});

// Task to delete target: contents of the vendor lib folder
gulp.task("clean", async function () {
  return del([
    "mysite/static/vendors/**",
    "!mysite/static/vendors",
    "!mysite/static/vendors/.gitignore",
  ]);
});

gulp.task("sass", function () {
  return gulp
    .src("mysite/static/scss/**/*.scss") // Gets all files ending with .scss in static/scss and children dirs
...
gulp.task("lint-css", function lintCssTask() {
  ...
  return gulp.src("mysite/static/scss/**/*.scss").pipe(
    gulpStylelint({
...
gulp.task("uglify", async function () {
  return gulp
    .src("mysite/static/js/!(*.min).js") // see https://stackoverflow.com/a/58470327
    .pipe(babel({ presets: ["@babel/preset-env"] }))
    .pipe(uglify())
    .pipe(rename({ suffix: ".min" }))
    .pipe(gulp.dest("mysite/static/js"));
});

// Minify CSS
gulp.task("minify", function () {
  return gulp
    .src("mysite/static/css/!(*.min).css") // grab everything that doesn't end with *.min.css
...
gulp.task("watch", function () {
  gulp.watch("mysite/static/scss/**/*.scss", gulp.series("sass"));
  gulp.watch("mysite/static/css/!(*.min).css", gulp.series("minify", "reload"));
  gulp.watch("mysite/static/js/!(*.min).js", gulp.series("uglify", "reload"));
  gulp.watch("mysite/**/*.html", gulp.series("reload"));
  gulp.watch("mysite/**/*.py", gulp.series("reload"));
});
